...
10. **The tweaks panel does not ship.** It's a design-time tool for comparing variants (paper vs field map theme, accent palettes, label density, annotation visibility, search-bar shape). Pick one default in production — recommended: paper theme, accent #ea580c/#9a3412, normal label density, annotations on, rounded search bar.
